...
DATA_URL=("C:/Users/asd/Desktop/project1/Tweets.csv")
...
# dropdown list
select = st.sidebar.selectbox('Visualization type',['Histogram','Pie chart'],key='1')#key is so the widget selectbox dosent confuse itself with other widget
...
# making a word WordCloud
st.sidebar.header("Word Cloud")
word_sentiment = st.sidebar.radio('Display word cloud for any of the below sentiment',('positive','neutral','negative'))

if not st.sidebar.checkbox("Close",True):#by default hidden
    st.header('Word cloud for %s sentiment'%(word_sentiment))
    df = data[data['airline_sentiment']==word_sentiment]
    words = ' '.join(df['text']) #add sapce btw words and join them
